...
React.useEffect allows you to run custom code each time React renders/re-renders your component. 

The useState setter function is called the `dispatch` function

### Lazy Initialization
You can pass a FUNCTION to useState. This is less common, but good for performance if your initial state is computationally expensive (such as retrieivng a value from local storage). You wouldn't want to retrieve the initial state from local storage every time the component is re-rendered, as you only need it when the component is FIRST rendered.

By passing useState a function, React will only call that function on the inital render:
```javascript
const getInitialState = () => Number(window.localStorage.getItem('count')) 
const [count, setCount] = React.useState(getInitialState)
```

Kent only uses the above feature 2% of the time, but it's good to know about.

### Function Updates

We can also pass a function to the `dispatch` function returned from useState. This is like passing a function to setState in class-based React. It takes the previous state value as an argument. Just like with setState, you use function updates any time you need to compute new state based on previous state. 

This works because instead of using the closed-over variable which can be "stale" or out of date, we use the current state which is always accurate.

This problem typically doesn't occur in useReducer, but in useReducer your state update may be dependent on props or some external state, in which case you may need to use useRef to make sure you have the most current version of that prop or external state. But that's a future topic.

### useEffect dependencies

useEffect callback is called EVERY time the component is re-rendered, not just when the state changes. luckily, we can pass a second argument, an array of DEPENDENCIES, which tells React to only call the callback function when those DEPENDENCIES change. 

### Custom Hooks
A custom Hook is a javascript function whose name starts with "use" and that may call other Hooks. 
The function must start with "use" so react can check for violations of rules of hooks
What makes a custom hook a custom hook is that it uses other hooks inside of it.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=whitleyblaine%40protonmail.com).
