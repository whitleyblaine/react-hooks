...
If you want to use async/await in useEffect, you can't pass an async function to useEffect directly, because it will return a Promise, not a cleanup function.
...
When destructuring objects as function params in es6, you can rename the object keys for use inside the function:
...
Note: error boundaries do NOT catch errors for event handlers, async code, server side rendering, errors thrown in the error boundary itself (rather than its children)

A class component becomes an error boundary if it defines either (or both) of the lifecycle methods static getDerivedStateFromError() or componentDidCatch(). Use static getDerivedStateFromError() to render a fallback UI after an error has been thrown. Use componentDidCatch() to log error information.

Then put your components inside it:
...
As of React 16, errors not caught by any error boundary result in an unmounting of the whole component tree.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=whitleyblaine%40protonmail.com).
