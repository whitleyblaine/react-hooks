...
Great definition for 'state': data that changes over time.

When we call the setter returned from React.useState, this tells React to re-render the component (call the function again). 
When it does this, the value NOW returned from useState will be the value we called the setter with.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=01%3A%20useState%3A%20greeting&em=whitleyblaine%40protonmail.com).
